...
- `src/pages/[city]/photo/[...filename].astro` — 写真詳細・EXIF 表示ページ（rest パラメータ。`.jpg` 等の拡張子付きセグメントは `[filename]` だとマッチしないため）
- `src/pages/404.astro` — カスタム 404 ページ
- `getStaticPaths()` で `pics.json` から全都市・全写真のパスを生成（`yusuke` を除外する）
- ヘッダー・ナビなど共通コンポーネントを Astro コンポーネントに移植
- OGP / メタデータ生成を `BaseLayout.astro` 経由に移植（`PUBLIC_HOST` は `import.meta.env` から取得）
- `src/pages/yusuke.astro` — **タブ枠 + career コンテンツを静的 HTML として出力、JS なし**（タブ切り替え・モーダル等のインタラクティブ機能は Phase 3 で Islands 化）

### 完了条件

- ギャラリー全ページ（`/`・`/:city`・`/:city/photo/:filename`）は現状と同等の表示・動作になる
- `/yusuke` はタブ枠 + career コンテンツが静的 HTML で配信されること（JS なし、タブ切替は Phase 3 まで不可）
...
- [ ] `/yusuke` が 404 にならずタブ枠 + career コンテンツの静的 HTML が配信されること
- [ ] 拡張子付き・日本語・スペース含むファイル名の写真詳細 URL が正しく解決されること
- [ ] 存在しない都市へのアクセスで 404 になること

### 備考

Phase 2 完了後に `src/index.tsx`（Hono エントリ）と旧 vite プラグイン群を削除し、`CLAUDE.md` と `generatePics/CLAUDE.md` を更新する。ロールバックが必要な場合は旧 Hono ブランチを保持しておく。写真追加手順（`generatePics` → `pics.json` 生成 → `npm run build` → デプロイ）は現状維持。`pics.json` はコミット対象だが、CI 導入時は `generatePics` → `build` の順を workflow に明記すること（`public/pics/` 実体が clone に含まれないため）。
